...
class Task:
    """
    TASK: help guide Turtle home, which will randomly appear
    every time the task starts
    """
    def start(self):
        stage.reset()
        stage.turtle.pos_changed_callbacks.append(self.pos_changed)
        self.home = Home(-1*randint(100,300),-1*randint(50,100),32,32,IMG_HOME)
        stage.add_object(self.home)
        self.boulders = [
            Boulder(-50, 0,25,IMG_BOULDER),
            Boulder(50, 0,25,IMG_BOULDER),
        ]
